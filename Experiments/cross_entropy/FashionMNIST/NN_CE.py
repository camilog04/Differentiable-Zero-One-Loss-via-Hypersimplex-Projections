...
def test_loss(input, target, regularization="kl", regularization_strength=1.0):
    # this is a place holder, only CE loss is used in this script
    pass
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for (img, label) in test_dl:
                img, label = img.to(args.device), label.to(args.device)
                logit = model(img)
                test_acc.update((logit.argmax(-1) == label).float().mean(), img.shape[0])
    ...
    regularization = (
        ...
        if args.loss_fn == "test_loss" else ""
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss_fn", choices=["cross_entropy", "test_loss"], default="cross_entropy")
