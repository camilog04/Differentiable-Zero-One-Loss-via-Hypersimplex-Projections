...
class MultiClassMSELoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        return self.mse(probs, target_onehot)
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for (img, label) in train_dl:
            ...
            pred = model(img)
            #target_multi = F.one_hot(label, num_classes=NUM_CLASSES).float()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--loss_fn", choices=["cross_entropy", "MSE"], default="MSE"
    )
