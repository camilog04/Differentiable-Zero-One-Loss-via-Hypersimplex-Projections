...
import logging
logging.basicConfig(
    filename="criteo_hypersimplex.log", 
    level=logging.INFO
)
...
infile_train = "../data/criteo_x1/train.libsvm"
infile_valid = "../data/criteo_x1/valid.libsvm"
...
def projected_hypersimplex_loss(pred, target):
    ...
    loss_grads = torch.autograd.grad(loss, pred)[0] * 0.001 # re-scaling by temperture
    

    loss_grads = loss_grads.to('cpu').detach().numpy().reshape(-1)
    
    #hessian = torch.autograd.functional.hessian(rank_diff_mse, (target, pred ) )
    #hessian = torch.autograd.functional.jacobian(rank_diff_mse, (target, pred ) )
...
res = {}
start = time.time()
logging.info("Hypersimplex traning starting (AUC, criteo)...")
bst = lgb.train(
    params, train_data, 
...
# with open('proj_classy_test.pkl', 'wb') as f:
#     pickle.dump(bst, f)
