...
import logging
logging.basicConfig(
    filename="higgs_hypersimplex.log", 
    level=logging.INFO
)
...
infile_train = "../data/higgs/higgs.train"
infile_valid = "../data/higgs/higgs.valid"
...
def projected_hypersimplex_loss(pred, target):
    ...
    loss_grads = torch.autograd.grad(loss, pred)[0]
...
res = {}
start = time.time()
logging.info("Hypersimplex traning starting (AUC, HIGGS)...")
bst = lgb.train(
    params, train_data, 
